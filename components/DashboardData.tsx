  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const fetchStats = async () => {
        ...
        const [activeRes, overdueRes, completedRes, upcomingRes] = await Promise.all([
          supabase
            .from("tasks")
    ...
    <View className="px-5 pb-5">
      <View className="flex-row gap-5 flex-wrap">
        <View className="flex-row justify-between w-[48%] rounded-3xl border border-blue-100 bg-blue-50 p-4">
          <View className="flex-1 gap-2">
            <Text className="text-sm font-medium text-blue-700">Active Task</Text>
            <Text className="text-2xl font-bold text-blue-950">{activeTasks}</Text>
          </View>
        ...
        <View className="flex-row justify-between w-[46%] rounded-3xl border border-rose-100 bg-rose-50 p-4">
          <View className="flex-1 gap-2">
            ...
            <Text className="text-2xl font-bold text-rose-950">{overdueTasks}</Text>
          </View>
          <View className="mt-1 h-11 w-11 items-center justify-center rounded-2xl bg-white">
        ...
        <View className="flex-row justify-between w-[48%] rounded-3xl border border-violet-100 bg-violet-50 p-4">
          <View className="flex-1 gap-2">
            <Text className="text-sm font-medium text-violet-700">Team Members</Text>
            <Text className="text-2xl font-bold text-violet-950">{teamMembers?.length ?? 0}</Text>
          </View>
          <View className="mt-1 h-11 w-11 items-center justify-center rounded-2xl bg-white">
        ...
        <View className="flex-row justify-between w-[46%] rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
          <View className="flex-1 gap-2">
            <Text className="text-sm font-medium text-emerald-700">Completed</Text>
            <Text className="text-2xl font-bold text-emerald-950">{completedTasks}</Text>
          </View>
          <View className="mt-1 h-11 w-11 items-center justify-center rounded-2xl bg-white">
      ...
      <View className="mt-5 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm shadow-black/5">
        <View className="flex-row justify-between items-center mb-3">
          <View>
            <Text className="text-lg font-bold text-gray-900">Upcoming Deadlines</Text>
            <Text className="text-sm text-gray-500">The next items needing attention</Text>
          </View>
          ...
          upcomingDeadlines.map((task) => (
            ...
            >
              <View className="flex-1 pr-3">
                <Text className="font-semibold text-base text-gray-900">{task.title}</Text>
                <Text className="text-sm text-gray-500">{task.priority ?? "Medium"} priority</Text>
              </View>
              <Text className="text-sm font-medium text-gray-600">{formatDeadline(task.deadline)}</Text>
            </View>
          ))
        ) : (
          <View className="items-center rounded-2xl bg-gray-50 px-4 py-6">
            <Ionicons name="sparkles-outline" size={22} color="#6B7280" />
            <Text className="mt-2 text-center text-sm text-gray-500">No upcoming deadlines.</Text>
          </View>
        )}
      </View>
    </View>
